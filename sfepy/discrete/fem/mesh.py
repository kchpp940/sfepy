...
class Mesh(Struct):
    ...
    def get_cell_groups(self, desc=None):
        """
        Get cell groups for a given element type or all types.

        Parameters
        ----------
        desc : str, optional
            Element type descriptor (e.g., '2_3', '3_4'). If None, return
            cell groups for all element types.

        Returns
        -------
        cell_groups : dict or array
            If desc is None, returns dict mapping desc -> cell_groups array.
            If desc is given, returns cell_groups array for that type.
        """
        if desc is None:
            return {d: self.get_cell_groups(d) for d in self.descs}
        else:
            cmesh = self.get_cmesh(desc)
            cells = nm.where(cmesh.cell_types == cmesh.key_to_index[desc])[0]
            return cmesh.cell_groups[cells]

    def set_cell_groups(self, groups, desc=None):
        """
        Set cell groups for a given element type or all types.

        Parameters
        ----------
        groups : dict or array
            If desc is None, dict mapping desc -> cell_groups array.
            If desc is given, cell_groups array for that type.
        desc : str, optional
            Element type descriptor (e.g., '2_3', '3_4').
        """
        if desc is None:
            for d, g in groups.items():
                self.set_cell_groups(g, d)
        else:
            cmesh = self.get_cmesh(desc)
            cells = nm.where(cmesh.cell_types == cmesh.key_to_index[desc])[0]
            cmesh.cell_groups[cells] = groups
    ...
    def get_vertex_groups(self):
        ...
        -------
        vertex_groups : array
            Array of vertex group IDs.
        """
        return self.cmesh.vertex_groups
    ...
    def set_vertex_groups(self, groups):
        ...
        groups : array
            Array of vertex group IDs.
        """
        self.cmesh.vertex_groups[:] = groups

    def get_unique_cell_groups(self):
        """
        Get all unique cell group IDs.

        Returns
        -------
        unique_groups : array
            Sorted array of unique cell group IDs.
        """
        all_groups = nm.concatenate([self.get_cell_groups(d) for d in self.descs])
        return nm.unique(all_groups)

    def get_unique_vertex_groups(self):
        """
        Get all unique vertex group IDs.

        Returns
        -------
        unique_groups : array
            Sorted array of unique vertex group IDs.
        """
        return nm.unique(self.cmesh.vertex_groups)

    def remap_cell_groups(self, remap_dict):
        """
        Remap cell group IDs according to a mapping dictionary.

        Parameters
        ----------
        remap_dict : dict
            Dictionary mapping old group IDs to new group IDs.
        """
        for desc in self.descs:
            groups = self.get_cell_groups(desc)
            for old_id, new_id in remap_dict.items():
                groups[groups == old_id] = new_id
            self.set_cell_groups(groups, desc)

    def remap_vertex_groups(self, remap_dict):
        """
        Remap vertex group IDs according to a mapping dictionary.
        ...
        remap_dict : dict
            Dictionary mapping old group IDs to new group IDs.
        """
        groups = self.cmesh.vertex_groups.copy()
        for old_id, new_id in remap_dict.items():
            groups[groups == old_id] = new_id
        self.cmesh.vertex_groups[:] = groups

    def get_group_mapping(self, desc=None):
        """
        Get cell groups as indices within each cell block for meshio export.

        Parameters
        ----------
        desc : str, optional
            Element type descriptor. If None, return for all types.

        Returns
        -------
        group_mapping : dict
            Dictionary mapping group ID to local indices within cell block.
            Format: {group_id: array_of_local_indices}
        """
        if desc is None:
            return {d: self.get_group_mapping(d) for d in self.descs}
        else:
            groups = self.get_cell_groups(desc)
            unique_groups = nm.unique(groups)
            return {str(g): nm.where(groups == g)[0] for g in unique_groups}

    def get_vertex_group_mapping(self):
        """
        Get vertex groups as indices for meshio export.

        Returns
        -------
        group_mapping : dict
            Dictionary mapping group ID to vertex indices.
            Format: {group_id: array_of_vertex_indices}
        """
        groups = self.cmesh.vertex_groups
        unique_groups = nm.unique(groups)
        return {str(g): nm.where(groups == g)[0] for g in unique_groups}
