...
def test_cell_vertex_group_methods():
    """
    Test the unified cell/vertex group methods on Mesh.
    """
    import sfepy
    from sfepy.discrete.fem import Mesh
    from sfepy import data_dir
    ...
    cell_groups = mesh.get_cell_groups()
    assert isinstance(cell_groups, dict), \
        'get_cell_groups() should return dict'
    for desc in mesh.descs:
        assert desc in cell_groups, \
            'dict should contain key for each element type'
        cg = cell_groups[desc]
        expected_n = mesh.get_conn(desc).shape[0]
        assert len(cg) == expected_n, \
    ...
    for old_val in remap_dict_v.keys():
        assert not nm.any(vg_remapped == old_val), \
            'remapped value should not appear'
    mesh.set_vertex_groups(vg)
